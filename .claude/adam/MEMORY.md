...
- **Vercel:** connected -- `https://fb-monetization-mvp-brainatlas-243p7ar66.vercel.app`. Project ID `prj_UUodrhr6KYPKOMpryQm8ghXQFWCo`, team ID `team_QYpVsJrdTZVIkWVkhhKrGEGG` (slug `noel-greganda-s-projects`). Env vars set (per Noel). **Confirmed broken (2026-09-15): Vercel's Git-push auto-deploy is not firing** -- the live site still showed pre-session content after 4+ real commits landed on `origin/main` (verified via `git log origin/main`, not assumed). Workaround shipped: `.github/workflows/deploy.yml` deploys via `vercel deploy --prod` CLI directly, triggered by GitHub Actions on push + `workflow_dispatch` -- proven to reach the Vercel CLI correctly (real error: `--token missing a value`), just needs `VERCEL_TOKEN`/`VERCEL_ORG_ID`/`VERCEL_PROJECT_ID` as GitHub secrets (not yet added). A Vercel MCP connector is also available in-session but returns 403 Forbidden for this team/project -- authenticated as some identity without access to it, not usable for this project as-is.
