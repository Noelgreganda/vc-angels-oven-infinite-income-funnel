...
- **New hard blocker, unrelated to Meta: Vercel Hobby plan's 100-deployments/day cap was hit mid-session.** `git push` for `fecbf28` triggered `deploy.yml` as always, but `vercel deploy` failed with `Error: Resource is limited - try again in 24 hours (more than 100, code: "api-deployments-free-per-day")`. This is cumulative across the whole day's iteration (this session pushed ~9 times), not caused by any single bad commit. **Practical consequence: do not push to `fb-monetization-mvp` main again until this resets (~24h from whenever the count started) or Noel upgrades to Pro** -- every push still triggers a real deploy attempt that will fail the same way and waste a cycle. Confirmed via the actual `vercel deploy` error text in CI logs, not assumed. Current live production commit is `7eb30b3` (has the compliance-gated Messenger auto-reply); `fecbf28` (brand-voice context) is merged to `main` on GitHub, fully built and locally verified (`tsc`/`eslint`/`next build` all clean), just not live -- redeploy it (`workflow_dispatch` on `deploy.yml`, or any commit) once the quota clears.
